...
use autograd::ndarray as ndarray;
use std::fs::File;
use std::io;
use std::io::Read;
use std::path::Path;
...
pub fn data_location(path: &str) -> AnyhowResult<PathBuf> {
    Ok(env::current_dir()?.join(Path::new("ml_data/mnist")).join(path))
}
...
pub fn load() -> ((NdArray, NdArray), (NdArray, NdArray)) {
    ...
    let (train_x, num_image_train): (Vec<f32>, usize) =
        load_images(data_location("train-images-idx3-ubyte").unwrap());
    let (train_y, num_label_train): (Vec<f32>, usize) =
        load_labels(data_location("train-labels-idx1-ubyte").unwrap());
    let (test_x, num_image_test): (Vec<f32>, usize) =
        load_images(data_location("t10k-images-idx3-ubyte").unwrap());
    let (test_y, num_label_test): (Vec<f32>, usize) =
        load_labels(data_location("t10k-labels-idx1-ubyte").unwrap());
